...
class PlaneSurface(NetworkGeometry):
	...
	def __init__(self, x, y=None, z=None origin=None):

		NetworkGeometry.__init__(self)

		self.

		# PlaneSurface parametric coords: (theta, h)
		self.parametricCoords		= numpy.full(shape=[0, PlaneSurface.numDimensions], fill_value=numpy.nan)
		# PlaneSurface surface plane coords: (x, y) relative to rectangular plane representing unwrapped plane surface
	...
	def calculate_distances(self):
		...
		for i in range(self.N):
			for j in range(self.N):
				...
				# Handle torroidal wrapping of plane plane or annulus rect. prism in x dimension for euclidean distance calculation:
			...
			if(numNeuronsToPosition != len(coords)):
				for i, coord in enumerate(coords):
					...
					if(len(coord) == PlaneSurface.numDimensions):
						...
						exitOnNetworkGeometryError("The dimensionality of given coordinate(s) ("+str(len(coord))+") does not match the parametric dimensionality of the geometry \'"+str(self.geometry)+"\' ("+str(PlaneSurface.numDimensions)+")")
			else:
