...
numReps	= 3
d_vals = numpy.arange(0.0, 2.5, 0.1)
c_vals = numpy.arange(0.0, 4.0, 0.2)
for idx_k, _CUR_GAP_D_THRESH_ in enumerate(d_vals):
	for idx_c, _CUR_GAP_C_W_ in enumerate(c_vals):
		for rep in range(numReps):
			counter += 1

			print "\n\n=============================="+str(counter)+"/"+str(len(d_vals)*len(c_vals)*numReps)+"\nd_thresh = "+str(_CUR_GAP_D_THRESH_)+"  c_w = "+str(_CUR_GAP_C_W_)+"  rep = " + str(rep+1)+"/"+str(numReps)
			...
			print "  spk rate: " + str(SPIKERATE)

			experimentData.append({'d_tresh':_CUR_GAP_D_THRESH_, 'c_w':_CUR_GAP_C_W_, 'rep':rep, 'spikeRate':SPIKERATE, 'numGapJns':NUMGAPJNS})
			...
			if(_CUR_GAP_D_THRESH_ == numpy.median(d_vals) and _CUR_GAP_C_W_ == numpy.sort(c_vals)[1] and rep == 0):
...
experimentDataframe.to_csv('Izh_torusSurface_evenlyPos_knnSynAdj_randuniformSynWts_knnGapAdj_constGapWts_1constInput_Tmax'+str(representativeNetwork.T_max)+'_'+str(numReps)+'reps_RESULTS.csv', index=False)

spikeRateAvgOverRepsDataframe = experimentDataframe['spikeRate'].groupby([experimentDataframe['c_w'], experimentDataframe['d_tresh']]).mean().unstack()
...
import seaborn as seaborn
seaborn.heatmap(data=spikeRateAvgOverRepsDataframe, annot=False, cmap='hot', ax=ax_avgRateHeatmap, yticklabels=(True if len(c_vals)<10 else int(len(c_vals)/2)-1), xticklabels=(True if len(d_vals)<10 else int(len(d_vals)/2)-1) )
...
# ax_avgRateHeatmap.xticklabels([numpy.min(d_vals), numpy.max(d_vals)])

experimentFigure.tight_layout()

pyplot.savefig('Izh_torusSurface_evenlyPos_knnSynAdj_randuniformSynWts_knnGapAdj_constGapWts_1constInput_Tmax'+str(representativeNetwork.T_max)+'_'+str(numReps)+'reps_RESULTS.png', bbox_inches='tight')
