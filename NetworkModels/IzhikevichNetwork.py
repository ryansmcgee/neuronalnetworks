...
class IzhikevichNetwork(NeuronNetwork):
...
#TODO: Check these are actually needed when fully implemented:
		...
		self.neuronLogVariables = ['neuron_id', 't', 'spike', 'V', 'U', 'I_excit', 'I_inhib', 'I_gap', 'I_input', 'g_excit', 'g_inhib', 'g_gap', 'synapse_type', 'label']
	...
	def network_update(self):
		...
		if(self.integrationMethod == 'euler' or self.integrationMethod == 'forwardeuler' or self.integrationMethod == 'rk1'):
			...
			dVdt 	= self.R_membrane*( self.k*(self.V - self.V_r)*(self.V - self.V_t) - self.U + (self.I_excit + self.I_inhib + self.I_gap + self.I_input) )
